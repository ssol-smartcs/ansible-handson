...
後ほど各演習内で説明がありますが、SmartCSは用途/操作に合わせて **ログインするユーザID** と **TCPポート番号** が異なります。
以下では、それぞれのケースにおけるパラメータ（ログインユーザ、TCPポート番号）を環境ごとに記載しています。

<br>
<br>

##### ① 手動での本体アクセス / Ansible経由でのコンソールアクセス
SmartCSでは装置にログインして設定変更や情報取得を行う事が可能なユーザと、<br>
SmartCSにシリアル経由で接続しているNW機器にアクセスできるユーザは分けて管理しています。<br>
**userXX**というユーザIDは拡張ユーザというユーザグループに所属しており、<br>
SmartCSに直接ログインする事と、Ansible経由でコンソールアクセス用のコマンドを実行する事の両方が可能です。<br>
下記のパラメータは、
...
| 環境ID | SmartCS IP | TCPポート | tty番号 | ユーザ ID | Password | 
|:---|:---|:---|:---|:---|:---|
...
##### ② ポートアクセス（手動）
SmartCSではあらかじめ設定された特定のポート番号を指定することで、直接目的の機器のコンソールに接続ができます。<br>
**portXX**というユーザIDはポートユーザというユーザグループに所属しており、<br>
SmartCSにシリアル経由で接続されているNW機器にログインする事ができるユーザとなります。
下記のパラメータは、**手動でSmartCSのポート番号を指定して、前述のAlaxalaのコンソール接続を行うため**のパラメータです。
...
| 9  | 192.168.129.**3**/24 | 8309 | 8419 | port09 | secret09 | 
...
異なるパラメータを指定する必要があります。<br>
（SmartCS用An）
下記のパラメータは、**AnsibleからSmartCS経由で他ベンダーモジュールを実行する場合**のパラメータです。
