...
STEP1では、<code>smartcs_tty_command</code>モジュールを使った簡単なPlaybookを作成します。<br>
このPlaybookでは、SmartCS経由でALAXALA装置にログインして<code>show version</code>を実行し、バージョン情報を取得します。<br>
<br>
参考として、Ansibleを使わない場合の一般的な操作と出力は、以下のようなオペレーションとなります。<br>
**[下記の操作は不要です]**
```
login: operator
...
■Playbook内容の説明
- hosts: smartcs  
このPlaybookを実施する対象として、インベントリの中のSmartCSを指定しています。"Alaxalaのコマンドを実行するのでAlaxala機器を選択したい"ところですが、SmartCS上でAlaxalaの操作をするため、AnsibleからはSmartCSを指定します。
- gather_facts: no  
AnsibleでPlaybookを実行する場合、デフォルトで動作する対象の情報を収集する機能をキャンセルしています。
- vars:
Playbook内だけで利用する変数を定義しています。
- tty_no: "{{ hostvars['ax']['smartcs_tty'] }}"   
Playbookで使用するtty_noという変数を定義しています。
多少難しい書き方をしていますが、**ax**というhostに設定されている**smartcs_tty**の値(hostvars)を**tty_no**の値として代入する、と書いてあります。
*smartcs_ttyは、デフォルトのインベントリファイルの中で定義されています。
- tty: {{ tty_no }}  
...
ワークショップの環境では、Alaxala機器は工場出荷時の状態となっていて、ユーザー名やパスワードもデフォルトとなっています。<br>
まずは、Alaxala機器の初期設定として、ユーザ名とパスワードの再登録をAnsible経由で実施します。<br>
