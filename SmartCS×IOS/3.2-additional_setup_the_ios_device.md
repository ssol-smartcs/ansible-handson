...
## 目次
本演習では以下を行います。 
- [Step 1. <code>ios_facts</code>を使って、IOS装置にAnsibleからアクセスできる事の確認を行う](./3.2-additional_setup_the_ios_device.md#step-1-ax_factsを使ってIOS装置にAnsibleからアクセスできる事の確認を行う)
...
- [Step 3. <code>ios_config</code>と<code>with_items</code>を使って、IOS装置に各種サーバの設定を行う（NTP、SNMP、SYSLOG）](./3.2-additional_setup_the_ios_device.md#step-3-ios_configとwith_itemsを使ってIOS装置に各種サーバの設定を行うNTPSNMPSYSLOG)
...
### Step 1. <code>ax_facts</code>を使って、IOS装置にAnsibleからアクセスできる事の確認を行う

演習3.1でAnsibleからリーチ可能となったALAXALA機器に対して、<code>ios_facts</code>を使って接続確認を行います。
...
### Step 3. <code>ios_config</code>と<code>with_items</code>を使って、IOS装置に各種サーバの設定を行う（NTP、SNMP、SYSLOG）  

次は<code>ios_config</code>と<code>with_items</code>利用して追加の設定を行います。  
