...
Playbookを実行したディレクトリに、`config_<iosのホスト名>_時刻情報.txt`というファイルができています。  
catコマンドなどで内容を確認して下さい。
```
$ ls -l
～抜粋～
-rw-rw-r-- 1 webinar webinar  4526 Jun 11 04:19 config_Cat3550_20210611_041927.txt
$
...
◇`name: check S/W version`  
- that: "'{{ os_version }}' in '{{ result.stdout_lines[0][0] }}'"   
`show version`コマンドの実行結果の1行目Cisco IOS Software, C3550 Software (C3550-IPSERVICESK9-M), Version 12.2(44)SE6, RELEASE SOFTWARE (fc1)"に、  
...
`show version`コマンドの実行結果の1行目にvarsで指定した`os_verion`が含まれている為、実行結果はOKとなりました。  
...
fatal: [ios]: FAILED! => {
    "assertion": "'12.0' in result.stdout_lines[0][0]",
    "changed": false,
    "evaluated_to": false,
