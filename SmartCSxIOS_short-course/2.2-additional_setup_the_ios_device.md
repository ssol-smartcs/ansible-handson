...
- [Step 3. <code>ios_config</code>と<code>loop</code>を使って、IOS装置に各種サーバーの設定を行う（NTP、SNMP、SYSLOG）](./2.2-additional_setup_the_ios_device.md#step-3-ios_configとloopを使ってIOS装置に各種サーバーの設定を行うNTPSNMPSYSLOG)
<br>
<br>
...
VLAN番号はvarsで定義した変数(vid)を参照しています。   
(vid)についてはshow running-configの記載と正確に合わせて下さい。
...
(portif)についてはshow running-configの記載と正確に合わせて下さい。
...
present in the running configuration on
device
changed: [ios]
...
### Step 3. <code>ios_config</code>と<code>loop</code>を使って、IOS装置に各種サーバーの設定を行う（NTP、SNMP、SYSLOG）  

次は<code>ios_config</code>と<code>loop</code>利用して追加の設定を行います。  
CLIの中で、一部の値（サーバーIPなど）のみ異なる設定を複数投入する場合などに有効な手法となります。  
設定する内容は以下となります。
- ホスト名の設定
- NTPサーバーの設定  
- SNMPサーバーの設定  
...
NTPサーバー、SNMPサーバー、SYSLOGサーバーの設定について、設定項目のパラメータを定義した変数ファイルを利用します。  
...
<code>host_vars/ios.yml</code>を利用し、各種サーバーを設定するPlaybookは以下の内容となります。  
