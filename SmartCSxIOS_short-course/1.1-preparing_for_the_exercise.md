...
#### 演習構成図
![practice_environment_1-1_rev2.png](./contents/image/practice_environment_1-1_rev2.png)
...
EC2への接続に必要なEC2 Global IP、ユーザー ID、Passwordはzoomのチャットにて別途お知らせします。  
EC2への接続情報は以降の演習で使用します。また、環境ID(1～10)は本説明の後に記載のIOS機器/SmartCSへの接続情報においても使用しますので、  
...
IOS機器への接続情報は以降の演習で使用しますので、ご自身の環境IDに該当する行のみコピーしてメモ帳などに控えてください。  
※他の参加者の情報で接続してしまうことの無いようご注意願います。  
※IOS装置のIPアドレスは演習2.1にて設定します。ハンズオン開始時点では初期状態（未設定）となります。  

| 環境ID | IOS IP | default ユーザー ID | ユーザー ID<br> (初期設定後) | Password<br> (初期設定後) | Enable Password<br> (初期設定後) |
|:---|:---|:---|:---|:---|:---|
...
SmartCSへの接続情報は以降の演習で使用しますので、ご自身の環境IDに該当する行のみコピーしてメモ帳などに控えてください。  
※他の参加者の情報で接続してしまうことの無いようご注意願います。  
...
SmartCSはSmartCS自体の設定変更や情報取得を行う事が可能なユーザー**＜一般ユーザー＞** と、<br>
SmartCSに接続されているNW機器のコンソールにアクセスできるユーザー**＜ポートユーザー＞** を通常分けて管理しています。<br>
**userXX**というユーザーは一般ユーザーでもポートユーザーでもない、**拡張ユーザー** というユーザーグループに所属しており、<br>
...
| 環境ID | SmartCS IP | TCPポート | tty番号 | 拡張ユーザー ID | Password | 
|:---|:---|:---|:---|:---|:---|
...
**portXX**というユーザーIDは**ポートユーザー**というユーザーグループに所属しており、<br>
SmartCSにシリアル経由で接続されているNW機器にログインする事ができるユーザーとなります。<br>
下記のパラメータは、
- **手動でSmartCSのポート番号を指定して、前述のIOSのコンソール接続を行う場合**<br>

に指定するパラメータとなります。

| 環境ID | SmartCS IP | TCPポート (RW) | TCPポート (RO) | ポートユーザー ID | Password | 
|:---|:---|:---|:---|:---|:---|
...
| 環境ID | SmartCS IP | TCPポート | ポートユーザー ID | Password | 
|:---|:---|:---|:---|:---|
...
- [環境構成図](./contents/pdf/document_Environment%20diagram_0924.pdf)

- [ユーザー毎環境](.//contents/pdf/document_Environment_for_each_user.pdf)
...
```bash
[webinar@ip-172-26-10-114 ~]$ source ~/ansible-handson/bin/activate
((ansible-handson) )[webinar@ip-172-26-10-114 ~]$ cd ~/ansible-handson
((ansible-handson) )[webinar@ip-172-26-10-114 ansible-handson]$
```
<br>
<br>
...
以下のような出力となるはずです。
```
((ansible-handson) )[webinar@ip-172-26-10-114 ansible-handson]$ ansible --version
ansible [core 2.18.3]
  config file = /home/webinar/.ansible.cfg
  configured module search path = ['/home/webinar/.ansible/plugins/modules', '/usr/share/ansible/plugins/modules']
  ansible python module location = /home/webinar/ansible-handson/lib64/python3.12/site-packages/ansible
  ansible collection location = /home/webinar/.ansible/collections:/usr/share/ansible/collections
  executable location = /home/webinar/ansible-handson/bin/ansible
  python version = 3.12.11 (main, Jul 30 2025, 00:00:00) [GCC 11.5.0 20240719 (Red Hat 11.5.0-5)] (/home/webinar/ansible-handson/bin/python3.12)
  jinja version = 3.1.6
  libyaml = True
((ansible-handson) )[webinar@ip-172-26-10-114 ansible-handson]$ 
```
Ansibleを使った演習については以下のAnsibleバージョンで行います。<br>
※Ansible-coreのバージョンが`2.18.3`である事を確認して下さい。
...
|:---|:---|
|Ansible-core | 2.18.3 |
|SmartCS module for Ansible | 1.7.0 |
|IOS modules for Ansible | 9.1.1 |
...
```bash
((ansible-handson) )[webinar@ip-172-26-10-114 ~]$ cat /home/webinar/.ansible.cfg
[defaults]
inventory       = ~/hosts
...
```bash
cat /home/webinar/hosts
```

<code>/home/webinar/hosts</code>（#1ユーザーの場合）  
  
```
((ansible-handson) )[webinar@ip-172-26-10-114 ansible-handson]$ cat /home/webinar/hosts
[seiko]
...
[controll]
ansible ansible_host=172.26.10.114 ansible_user=webinar ansible_password=XXXXX
...
smartcsはseikoグループに所属し、 ipアドレス: 192.168.129.1 , ユーザーID: user01 , パスワード: secret01として設定されています。<br>
...
※ユーザー#1の場合はuser01/secret01となります。  
※演習1.1に記載の[環境情報](./1.1-preparing_for_the_exercise.md#コンソールサーバ--smartcs-)を参考にして拡張ユーザーのID/パスワードを入力してください。  
※SmartCSのプロンプト先頭の括弧内の数字は固定ではなく、変動する可能性があります。（接続のたびに空いている番号から割り振られます。）
<br>

■SmartCS（NS-2250-48）
```
((ansible-handson) )[webinar@ip-172-26-10-114 ansible-handson]$ ssh userXX@smartcs
